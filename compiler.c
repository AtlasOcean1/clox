...
static Token sytheticToken(const char* text) {
  Token token;
  token.start = text;
...
static void super_(bool canAssign) {
  ...
  namedVariable(sytheticToken("this"), false);
  if (match(TOKEN_LEFT_PAREN)) {
    uint8_t argCount = argumentList();
    namedVariable(sytheticToken("super"), false);
    emitBytes(OP_SUPER_INVOKE, name);
    emitByte(argCount);
  } else {
    namedVariable(sytheticToken("super"), false);
    emitByte(OP_GET_SUPER, name);
  }
}
